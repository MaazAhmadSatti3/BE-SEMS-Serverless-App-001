...
// src/functions/getListOfEnrollments/handler.ts
__export(exports, {
  main: () => main
});
...
var getAllEnrollments = async (query) => {
  try {
    const enrollments = await dynamoClient.scan(query).promise();
    if (!enrollments)
      throw Error(`No enrollments found`);
    return enrollments;
  } catch (error) {
    console.error(error);
    return {
...
// src/functions/getListOfEnrollments/handler.ts
var getListOfEnrollments = async (event) => {
  let query = {
    TableName: "SEMSCRUD",
...
var main = middyfy(getListOfEnrollments);
