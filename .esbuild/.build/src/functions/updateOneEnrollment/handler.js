...
var dynamoClient = process.env.IS_OFFLINE ? new AWS.DynamoDB.DocumentClient({
  region: "localhost",
  endpoint: "http://localhost:8000"
}) : new AWS.DynamoDB.DocumentClient({});
var updateCourse = async (query) => {
  try {
...
var isEnrollExist = async (validation) => {
...
// src/functions/updateOneEnrollment/handler.ts
var updateEnrollmentH = async (event) => {
  ...
  if (validCourse) {
    ...
    const query = {
      ...
      FilterExpression: "SET courseid = :courseid, studentid = :studentid, dateofassigment = :dateofassigment",
...
var main = middyfy(updateEnrollmentH);
